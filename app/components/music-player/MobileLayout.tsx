  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 min-h-0 px-5 pt-5 relative z-20">
        ...
        >
          <div className="relative w-full h-full rounded-3xl overflow-hidden bg-neutral-800">
            {imageUrl ? (
              <Image
                src={imageUrl}
            ...
            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent pt-20 pb-6 px-6 text-center">
              <p className="text-2xl font-bold truncate">{currentSong?.title ?? "Select a song"}</p>
      ...
      {/* Bottom controls area — above AudioBackground (z-10) */}
      <div className="shrink-0 px-6 pt-5 pb-8 flex flex-col gap-4 relative z-20">
        ...
        <div className="flex justify-between">
          <span className="text-xs text-neutral-500 tabular-nums">{formatTime(currentTime)}</span>
        ...
        <div className="flex items-center justify-between px-2 mt-2">
          <button onClick={cycleLoopMode} type="button" aria-label="Shuffle" className="text-neutral-400 p-2 hover:text-white transition-colors">
            <ShuffleIcon size={22} />
          </button>
          <button onClick={playPrev} type="button" aria-label="Previous" className="text-neutral-400 p-2 hover:text-white transition-colors">
            <PrevIcon size={22} />
          </button>
          ...
          <div className="relative w-20 h-20 flex items-center justify-center">
            ...
            <button
              ...
              className="w-16 h-16 rounded-full bg-pink-500 text-white flex items-center justify-center hover:bg-pink-600 transition-colors relative z-[1] disabled:opacity-50"
          ...
          <button onClick={playNext} type="button" aria-label="Next" className="text-neutral-400 p-2 hover:text-white transition-colors">
            <NextIcon size={22} />
          </button>
          <button
            ...
            className={`relative p-2 transition-colors ${loopMode !== "none" ? "text-pink-500" : "text-neutral-400 hover:text-white"}`}
          >
            <LoopIcon size={22} />
